...
Route::get('/new-saiful/webhook-info', function () {
    return view('new-saiful.webhook-info');
})->name('new_saiful.info');

/* ---------- Incoming payment webhook (no auth) ---------- */
Route::post('/api/payment/webhook', [PaymentController::class, 'incomingWebhook'])
...
Route::middleware(AuthUser::class)->group(function () {
    ...
    Route::post('add-funds/deposit', [PaymentController::class, 'deposit'])->name('deposit');
    Route::get('payment', [PaymentController::class, 'payment'])->name('payment');
    Route::get('payment/cancel', [PaymentController::class, 'payment_cancel'])->name('cancel.payment');

    Route::get('payment/success', [OrdersController::class, 'paymentSuccess'])->name('payment.success');
    Route::post('topup/buynow', [OrdersController::class, 'buynow'])->name('topup.buynow');

    /* ---------- NEW: Manual payment ---------- */
    Route::get('payment/{id}', [PaymentController::class, 'manualPayment'])->name('payment.manual');
    Route::post('payment/{id}/submit', [PaymentController::class, 'submitManualPayment'])->name('payment.manual.submit');

    Route::post('referral/claim', [ReferralController::class, 'claim'])->name('referral.claim');
    Route::post('referral/redeem', [ReferralController::class, 'redeemCoins'])->name('referral.redeem');
    Route::get('account/refer', [ReferralController::class, 'refer'])->name('account.refer');
