...
{  
    ...
    {  
        ...
        // Non-wallet -> create a pending order then redirect to manual /payment/{id}
        try {
            $orderData['status'] = Status::PENDING;
            $order = Order::create($orderData);

            $redirect = route('payment.manual', ['id' => $order->id]) .
                '?page=checkout&amount=' . $order->amount . '&order_id=' . $order->id;
