...
class HomeController extends Controller
{
    public function home() {
      // Cache heavy reads for 60s — drastically reduces DB load and page TTFB.
      $cache = \Illuminate\Support\Facades\Cache::class;
      return view('pages.home', [
        'categorys' => $cache::remember('home.categorys', 60, fn() =>
            Categorie::where('status', 1)->orderBy('slot', 'asc')->get()),
        'products'  => $cache::remember('home.products', 60, fn() =>
            Product::where('status', 1)->orderBy('slot', 'asc')->get()),
        'sliders'   => $cache::remember('home.sliders', 60, fn() =>
            Slider::where('status', 1)->orderBy('order_column', 'asc')->get()),
        'orders'    => Order::with('user:id,name,picture')->latest()->take(5)->get(),
      ]);
    }
